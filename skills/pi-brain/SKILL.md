...
- `creative-thinking {cues: [2-3], prompt?}` — **post-think fuse** (STRICTLY after `think`): takes latest `think {goal, hypotheses}` + `cues[2..3]` episodes → novel variant not in either source. If no prior `think`, hint “call think first” and synthesizes from episodes only. Delegates recall ranking. Prompt e.g. `neon + login → glass login` NOT `creative approach`; loose/missing auto-enriched with `cues+think goal`.
...
- Strict (`on`): 7-rule workflow enforced by extension (not docs): **Happy (2-call floor): recall → think → [creative-thinking if novel] → plan #1 → Turn1 read×N parallel → Turn2 edit×N+write×N+bash parallel → plan #2 done:[all] → remember → habit → git commit**
  **Unhappy (3-call floor): same but triggers ONLY when `brainStrict && activePlan && !isPlanDone && tool in [write,edit,bash] && (isError || bash logical fail: output /\b(fail|error|ENOENT|not found|cannot)\b/ without /\b(passed|success)\b/)` → failure → think{goal:'debug <failed Task N: tool:err>', hypotheses:[root cause, fix]} → plan #2 (must update plan{id,…}) → retry Turn1/Turn2 → plan #3 done:[all] → remember (enforced: `tool_call` blocks write/edit/bash until debug-think, then until plan update). Ad-hoc failures with no active plan do NOT trigger.**
  **Batch: 1 LLM call = N tool calls. Turn1 read×N; Turn2 edit×N+write×N+bash. Chunk edits: 1 edit/file, exact oldText, merge nearby. If oldText known → 1 call. Record → 0-call replay. 5-Step: Question→Delete→Simplify→Accelerate→Automate. // ponytail: deleted readCache, add per-path cache if throughput matters**
  ...
  3. **Creative-thinking-only-for-novelty (post-think)** — prompt instructs: `creative-thinking` for creative/novel tasks, skip for CRUD/bugfix; **STRICTLY after `think`** (workflow: `think → [creative-thinking if novel] → plan`). Fuses `think` hypotheses × retrieved episode patterns into novel variant (delegates `rankedForQuery`). If called without prior `think`, hint “call think first”.
