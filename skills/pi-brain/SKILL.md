...
- `creative-thinking {cues: [2-3], prompt?}` — **post-think creative layer** (STRICTLY after `think`): 1 layer of creativity on top of `think` → takes latest `think {goal, hypotheses}` + optional `cues[2..3]` episodes → novel variant not in think alone. Episodes are **optional enhancement** — if `recall(cues)` hits, fuse them; if no episodes match, synthesizes deliberation-only (recombines/twists hypotheses). If no prior `think`, hint “call think first”. Prompt e.g. `neon + login → glass login` NOT `creative approach`; loose/missing auto-enriched with `cues+think goal`.
...
- Strict (`on`): 7-rule workflow enforced by extension (not docs): **Happy (2-call floor): recall → think → [creative-thinking if novel] → plan #1 → Turn1 read×N parallel → Turn2 edit×N+write×N+bash parallel → plan #2 done:[all] → remember → habit → git commit**
  ...
  **Batch: 1 LLM call = N tool calls. Turn1 read×N; Turn2 edit×N+write×N+bash. Chunk edits: 1 edit/file, exact oldText, merge nearby. If oldText known → 1 call. Record → 0-call replay. 5-Step: Question→Delete→Simplify→Accelerate→Automate. // ponytail: deleted readCache, add per-path cache if throughput matters**
  ...
  3. **Creative-thinking-only-for-novelty (post-think creative layer)** — prompt instructs: `creative-thinking` for creative/novel tasks, skip for CRUD/bugfix; **STRICTLY after `think`** (workflow: `think → [creative-thinking if novel] → plan`). 1 layer on top of `think` — synthesizes novel variant from deliberation hypotheses alone; `cues[2..3]` episodes are optional enhancement (fused if `recall` hits, otherwise deliberation-only). If called without prior `think`, hint “call think first”. Delegates `rankedForQuery` when episodes exist.
