...
- `creative-thinking {cues: [2-3], prompt?}` — **post-think creative layer** (STRICTLY after `think`): 1 layer on top of `think` → takes latest `think {goal, hypotheses}` + optional `cues[2..3]` episodes → **3 forced variants** (each concrete, not in think/episodes alone). Episodes are **optional enhancement** — fused if `recall(cues)` hits, else deliberation-only SCAMPER (Substitute/Combine/Invert) + `⚠️ low-value warning` for CRUD. If no prior `think`, hint “call think first”. Contract: Variant 1=Substitute, 2=Combine, 3=Invert/Anti-pattern. Prompt e.g. `neon + login → glass login` NOT `creative approach`.
