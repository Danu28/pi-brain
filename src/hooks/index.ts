...
import { encodeAutoEpisode } from "./auto-encode";
import { isBashLogicalFail, isRmRfCommand } from "./guards";
import { nudgeRule5 } from "./lifecycle";
...
export function registerHooks(pi: ExtensionAPI) {
  ...
  pi.on("tool_result" as any, async (ev: any, ctx: any) => {
    ...
    try {
      if (["write", "edit"].includes(ev?.toolName) && ev?.input?.path) {
        ...
        const rel = p.startsWith(cwd)
          ? p.slice(cwd.length).replace(/^[/\\]+/, "")
          : p;
    ...
    if (["edit", "write"].includes(ev?.toolName) && !ev?.isError) {
      ...
      if (/passed|success|fixed|done|ok/i.test(output) && output.length > 20) {
        ...
        const best = [...brain.episodes.values()]
          .filter((e) => e.source === "remember")
          .map((e) => ({ e, s: scoreBase(e, cmd) }))
          .filter((x) => x.s > 0)
    ...
    if ((ev?.toolName === "remember" || ev?.toolName === "habit") && !ev?.isError) {
      ...
      const blocked =
        (ev as any)?.details?.blocked === true ||
        (ev as any)?.result?.blocked === true;
      if (!blocked) {
    ...
    // logical bash fail = output contains fail/error without success, even if isError false (tests, build)
    {
      ...
      if (brain.brainStrict && hasActivePlan && isMutation) {
        ...
        if (isFailure) {
          ...
          if (brain.consecutiveFailures < 2) {
            try {
              (ctx as any)?.ui?.notify?.(
                "First failure (1/2) — next continuous mutation failure will trigger unhappy path. Consider think{goal:'debug ...'} early.",
                "warning",
              );
            } catch {}
            const hint =
            ...
            return {
              content: [{ type: "text", text: truncate(cur + hint) }],
            } as any;
          }
          ...
          try {
            (ctx as any)?.ui?.notify?.(
              "Strict unhappy path: 2 continuous failures — call think{goal:'debug <Task N: " +
                (ev.toolName +
                  ":" +
                  (ev.input?.path ?? ev.input?.command ?? "").toString().slice(0, 30)) +
                "', hypotheses:[cause,fix]} before retry.",
              "warning",
            );
          } catch {}
          const hint =
            "\n[ brain: 2 continuous failures → rethink — call think{goal:'debug <failed Task N: " +
            ev.toolName +
            ">', hypotheses:[root cause, fix]} then plan{id,done} before retry ]";
          const cur = ev.content?.[0]?.text ?? "";
          return {
            content: [{ type: "text", text: truncate(cur + hint) }],
          } as any;
  ...
  pi.on("tool_call" as any, async (ev: any, ctx: any) => {
    if (!brain.brainStrict) {
      ...
      if (["remember", "recall", "think", "creative", "plan", "habit"].includes(ev.toolName)) {
        return {
          block: true,
          reason:
            "pi-brain is OFF — run /pi-brain on to enable. No injection, footer, or auto-encode active.",
        } as any;
      }
    ...
    if (ev?.toolName === "bash") {
      ...
      if (isRmRfCommand(cmd)) {
        if (!ctx?.hasUI)
          return {
            block: true,
            reason: "Blocked by brain guard: rm -rf needs UI confirm",
          } as any;
        try {
    ...
    if (
      brain.brainStrict &&
      brain.needsDebugThink &&
      ["write", "edit", "bash"].includes(ev?.toolName)
    ) {
      return {
        block: true,
        reason:
          "Blocked by strict unhappy path: failure occurred — call think{goal:'debug <failed Task N>', hypotheses:[root cause, fix]} before retry.",
      } as any;
    }
    if (
      brain.brainStrict &&
      brain.needsPlanUpdate &&
      ["write", "edit", "bash"].includes(ev?.toolName)
    ) {
      return {
        block: true,
        reason:
          "Blocked by strict unhappy path: debug think done — now update plan{id,done} before retry.",
      } as any;
    }
    ...
    if (brain.brainStrict && (ev?.toolName === "write" || ev?.toolName === "edit")) {
      if (!brain.thinkSatisfied) {
        return {
          block: true,
          reason:
            "Blocked by strict workflow Rule 2: call think{goal,hypotheses} before write/edit. Deliberate 2-3 approaches first.",
        } as any;
      }
    }
