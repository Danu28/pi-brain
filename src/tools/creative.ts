...
async function creativeThinkingExecute(_id: any, params: any, signal: any) {
  ...
  if (!recentThink) {
    // strict sequencing: creative must follow think — surface hint but still allow synthesis from episodes alone
    const hint = "[hint: creative is post-think — call think{goal, hypotheses} first so synthesis fuses hypotheses × episodes; proceeding with episodes only]\n";
    if (!unique.length) return { content: [{ type: "text", text: "No episodes found for cues and no think yet. Call think first, then creative with 2-3 cues." }], details: { episodes: [] } };
    ...
    const text2 = `${hint}${synthesisPrompt2}\n\n${sources2}\n\n→ Call think first, then re-run creative to fuse hypotheses × episodes into a variant not in either source.`;
  ...
  if (!unique.length) {
    ...
    const warning = `⚠️ [low-value warning: deliberation-only — no episodes matched cues "${params.cues.join(", ")}" — this is paraphrasing think; real value comes from think × episodes. For CRUD, skip creative and improve think instead.]`;
