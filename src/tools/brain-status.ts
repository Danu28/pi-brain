...
export function registerBrainStatus(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_id, _p, _sig, _upd, ctx: any) {
      ...
      // C6 prune transparency: exactCueIndex size shows O(1) index health + memo stats
      const idxStats = `Index: ${brain.tokenIndex.size} tokens → ${count} episodes (${remCount} remember, ${autoCount} auto) | cueIndex:${brain.exactCueIndex.size} | memo hits:${brain.memoHits} miss:${brain.memoMisses}`;
      ...
      const knobs = `Knobs: MAX_BYTES=${MAX_BYTES} MAX_LINES=${MAX_LINES} TAG_BOOST=${TAG_BOOST} HALF_LIFE=${HALF_LIFE_FACTOR}/${HALF_LIFE_DAYS}d REMEMBER_BOOST=${REMEMBER_BOOST} AUTO_BOOST=${AUTO_BOOST} TTL=${AUTO_TTL_MS/86400000}d PRUNE_WARN=${PRUNE_WARN} PRUNE_CAP=${PRUNE_CAP} BUDGET=${BUDGET_WARN_PCT}/${BUDGET_STOP_PCT}%`;
