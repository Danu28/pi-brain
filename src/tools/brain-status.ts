...
export function registerBrainStatus(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_id, _p, _sig, _upd, ctx: any) {
      ...
      const knobs = `Knobs: MAX_BYTES=${MAX_BYTES} MAX_LINES=${MAX_LINES} TAG_BOOST=${TAG_BOOST} HALF_LIFE=${HALF_LIFE_FACTOR}/${HALF_LIFE_DAYS}d REMEMBER_BOOST=${REMEMBER_BOOST} AUTO_BOOST=${AUTO_BOOST} TTL=${AUTO_TTL_MS/86400000}d PRUNE_WARN=${PRUNE_WARN} PRUNE_CAP=${PRUNE_CAP}`;
