...
export function registerTools(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptSnippet: "Persist a decision or fix to memory by cue (remember)",
    promptGuidelines: ["Use remember to encode durable decisions/fixes; use recall to find them later. Remember should be explicit — no hidden auto-encoding."],
    ...
    async execute(_id, params, _signal, _onUpdate, _ctx) {
      ...
      if (exact && !params.force) {
        ...
        brain.episodes.set(exact.id, exact); await saveMemory();
        (pi as any).events?.emit?.("brain:episode:encoded", exact);
        return { content: [{ type: "text", text: `Updated (audit: exact cue exists) ${exact.id} — was duplicate cue, merged instead of new | relevance ${formatRelevance(rel.score)} — ${rel.reasons.join(", ")} | tip: reuse cue to update, or rename cue to "${params.cue}-v2" for new variant` }], details: { id: exact.id, episode: exact, audit: "exact-cue-upsert", relevance: rel } };
      }
      ...
      if (scored.length && !params.force && !exact) {
        ...
        return { content: [{ type: "text", text: `Audit: ${scored.length} similar episode(s) found — not encoded.\n${preview}\n→ To update existing, reuse its cue. To force new, call remember again with force:true or rename cue to "${params.cue}-v2" | relevance ${formatRelevance(rel.score)}` }], details: { audit:"similar-found", similar:scored.map(x=>({episode:x.e,score:x.s})), relevance: rel, blocked:true } } as any;
      }
      ...
      await (pi as any).appendEntry?.("brain:episode", ep); brain.episodes.set(ep.id, ep); indexEpisode(ep); await saveMemory();
      (pi as any).events?.emit?.("brain:episode:encoded", ep);
      // Habit due signal — 2nd similar cue: if this cue family now has ≥2 encodes, hint habit
      const familyHits = [...brain.episodes.values()].filter(e=> e.cue.toLowerCase().includes(params.cue.toLowerCase().slice(0,4)) || scored.some(s=> s.e.id===e.id)).length;
      let habitHint = "";
      if (scored.length===1 || familyHits>=2) {
        const habitName = params.cue.toLowerCase().replace(/[^a-z0-9-]/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"").slice(0,24);
  ...
  pi.registerTool({
    ...
    promptSnippet: "Recall past episodes by cue (TF-IDF), filters tags/source/since, batch queries[]",
    promptGuidelines: ["Use recall to pattern-complete from memory when the task resembles past work (before recalling, think whether prior fixes exist)."],
    parameters: Type.Object({
    ...
    async execute(_id, params, signal) {
      ...
      if(allQToks.length&&brain.tokenIndex.size){
        ...
        if(idSets.length){
          ...
          const hits=[...hitIds].map(id=>brain.episodes.get(id)).filter(Boolean) as BrainEpisode[]; if(hits.length) candidates=hits; else if(brain.episodes.size>10) candidates=[];
        }
      }
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal) {
      ...
      await (pi as any).appendEntry?.("brain:deliberation", entry); brain.thinkSatisfied=true; brain.needsDebugThink=false;
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal) {
      if(params.id&&brain.plans.has(params.id)){
        ...
        if(params.done?.length) {
          for(const i of params.done) {
            ...
            if(t.depends?.length) {
              ...
              if(blocked) {
                const depTitles = t.depends.map(d=> `T${d+1} "${(pl.tasks[d]?.title||"").slice(0,40)}"`).join(", ");
                return {content:[{type:"text",text:`Blocked: Task ${i+1} "${t.title.slice(0,60)}" depends on [${t.depends.map(d=>d+1).join(",")}] (${depTitles}) not done yet — verifiable DAG — run plan{id:"${pl.id}",done:[${t.depends[0]}]} first`}],details:{error:"depends not satisfied", task:i}} as any;
              }
            }
            t.done=true;
          }
        ...
        pl.ts=Date.now(); brain.cachedLatestPlan=pl; brain.hasPlan=true;
  ...
  pi.registerTool({
    ...
    promptSnippet: "Scaffold a repeatable fix as .pi/skills/brain-<name>/SKILL.md",
    promptGuidelines: ["Use habit after the 2nd repeat of the same remember (repeated correction) to turn it into a reusable skill."],
    parameters: Type.Object({
    ...
    async execute(_id, params, signal, _upd, ctx: any) {
      ...
      try{ const {mkdirSync,writeFileSync}=await import("node:fs"); if(signal?.aborted) throw new Error("aborted"); mkdirSync(dir,{recursive:true}); await withFileMutationQueue(file, async()=>{writeFileSync(file,body,"utf8");}); }catch(e:any){ return {content:[{type:"text",text:`Failed: ${e.message}`}],details:{error:String(e)}}; }
  ...
  pi.registerTool({
    ...
    promptSnippet: "Show brain memory usage: episodes, tokens, overload signal",
    promptGuidelines: ["Use brain-status to check memory load (episodes/tokens) before deciding to prune or compact."],
    parameters: Type.Object({
      verbose: Type.Optional(Type.Boolean({ description: "Show full gist + knobs (default collapsed)" })),
    }),
    async execute(_id, params, _sig, _upd, ctx: any) {
      ...
      const pct:number|null = usage?.percent ?? null, overloaded=(pct!==null&&pct>80)||count>50;
      if(overloaded) (pi as any).events?.emit?.("brain:overload",{episodes:count,percent:pct});
      const idxStats=`Index: ${brain.tokenIndex.size} tokens → ${count} episodes (${remCount} remember, ${autoCount} auto) | memo hits:${brain.memoHits} miss:${brain.memoMisses} gen:${brain.memoGen}`;
      ...
      const verboseLine = `Stats: prune:${brain.stats.prune} block:${brain.stats.block} dedup:${brain.stats.dedup} nudge:${brain.stats.nudge} audit:${brain.stats.audit} | ${isVerbose() ? "PI_BRAIN_VERBOSE=1" : "verbose off (set PI_BRAIN_VERBOSE=1 or verbose:true)"}`;
      const lines = [
        `Episodes: ${count} (${remCount} remember, ${autoCount} auto)`,
        `Tokens: ${usedToks??"?"} / ${totalToks??"?"}${pct!==null?` (${pct}%)`:""}${overloaded?"\n[overload: consider compaction/pruning]":""}`,
