...
import { brain, isVerbose, renderPlan, saveMemory } from "./state";
...
export function registerTools(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, _ctx) {
      ...
      if (exact && !params.force) {
        ...
        brain.episodes.set(exact.id, exact); brain.recallMemo.clear(); saveMemory();
      ...
      await (pi as any).appendEntry?.("brain:episode", ep); brain.episodes.set(ep.id, ep); indexEpisode(ep); brain.recallMemo.clear(); saveMemory();
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal) {
      ...
      if(params.id&&brain.plans.has(params.id)){
        ...
        await (pi as any).appendEntry?.("brain:plan", pl); saveMemory();
      ...
      brain.plans.set(pl.id, pl); brain.cachedLatestPlan=pl; brain.needsPlanUpdate=false; brain.hasPlan=true; saveMemory();
  ...
  pi.registerTool({
    ...
    async execute(_id, _p, _sig, _upd, ctx: any) {
      ...
      const verboseLine = `Stats: prune:${brain.stats.prune} block:${brain.stats.block} dedup:${brain.stats.dedup} nudge:${brain.stats.nudge} audit:${brain.stats.audit} | ${isVerbose() ? "PI_BRAIN_VERBOSE=1" : "verbose off (set PI_BRAIN_VERBOSE=1)"}`;
