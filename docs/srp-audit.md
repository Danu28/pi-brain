...
| **creative-thinking** `src/tools/creative.ts` | Fuse 2-3 distant episodes + latest think into novel approach | CandidatePool + scoring (reimplements recall), prompt auto-enrichment (loose vs detailed), gist formatting | ⚠️ **Overlap** — reimplements `recall` scoring; should delegate to `candidatePool`+`scoreEpisode` only |
