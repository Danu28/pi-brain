...
| **creative-thinking** `src/tools/creative.ts` | **Declared:** “fuse distant episodes + latest think into novel approach” **Code:** loop `cues[2..3]` → `candidatePool(q)` + `scoreEpisode` + filter `>0` + sort + slice 2 → `pooled`, dedup `Map`, `recentThink` from `brain.deliberations`, `isLoose` prompt check (<15 chars or `creative approach`) → `synthesisPrompt` enrich, `gistForEpisode` sources, `context` = sources+deliberation, final fuse instruction. | **Fuse** — synthesize prompt + context from 2–3 cues + latest think. Verb: *fuse* | Prompt auto-enrich (`isLoose` → append `cues+thinkGoal`), `deliberationBlock` fetch, final `→ Combine insights` instruction, `truncate`. | **Candidate scoring loop** → `recall.rankedForQuery(query,2)` helper (instead of reimplementing `candidatePool+scoreEpisode`). **Gist** → `gist.ts`. | No — `cues[2..3],prompt?` unchanged. Prompt enrichment stays (tool-specific). |
