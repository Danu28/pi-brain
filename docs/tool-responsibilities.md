...
**One tool, one verb:** `remember` encodes · `recall` retrieves · `think` deliberates · `creative` fuses · `plan` sequences · `habit` scaffolds · `brain-status` reports. Helpers own scoring, tokenizing, gist, validation, indexing.
