...
|18| **Tool-call guards (blocking)** | `tool_call` blocks `plan` if `!thinkSatisfied` (strict — earliest gate, fires on the first pre-edit tool instead of the write/edit batch), blocks `write/edit` if `!thinkSatisfied` / `!hasPlan`. Batch-aware preflight: pending sibling `think`/`plan` calls in the same assistant message count as satisfied (pi preflights all siblings before any executes → compliant batched models never blocked). Duplicate blocks within one batch are terse. Also `needsDebugThink` (2 consecutive failures) + `rm -rf` guard with `ui.confirm` | `src/hooks.ts` | `think` / `plan` / `bash` | Returns `{block:true, reason:...}` + `ui.notify` warning — visible as blocked tool call | Block reason only in tool_call rejection, not in transcript history |
