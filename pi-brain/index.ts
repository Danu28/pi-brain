...
// T10 token estimator
function estTokens(s: string): number { return Math.ceil(s.length / 3.5); } // ponytail: 4→3.5 code-heavy estimate
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, _ctx) {
      ...
      // similarity audit: T1 decay-exempt (scoreBase) + IDF + source filter — ponytail: 3→5 cuts false positives
